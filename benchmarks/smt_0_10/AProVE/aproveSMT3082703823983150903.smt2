(set-info :smt-lib-version 2.6)
(set-logic QF_NIA)
(set-info :source |AProve team, see http://aprove.informatik.rwth-aachen.de/, submitted for SMT-COMP 2014|)
(set-info :category "industrial")
(set-info :status unsat)
(declare-fun b__8 () Int)
(declare-fun a__3 () Int)
(declare-fun a__5 () Int)
(declare-fun a__7 () Int)
(declare-fun a__4 () Int)
(declare-fun a__6 () Int)
(assert (and (>= (+ 1 (* (- 0 1) b__8)) 0) (= (+ (* b__8 a__3) (* (- 0 1) a__3)) 0) (>= (+ (* b__8 a__5 a__7) (* (- 0 1) b__8)) 0) (>= (+ (* a__4 a__3) (* (- 0 1) (* a__6 a__3)) (* (- 0 1) (* a__4 a__7 a__3)) (- 0 1)) 0) (>= (+ (* a__5 a__3) (* (- 0 1) (* a__5 a__7 a__3))) 0) (>= b__8 0) (>= 1 b__8) (>= a__3 0) (>= a__5 0) (>= a__7 0) (>= a__4 0) (>= a__6 0)))
(assert (<= a__3 10))
(assert (<= a__4 10))
(assert (<= a__5 10))
(assert (<= a__6 10))
(assert (<= a__7 10))
(check-sat)
;;(get-assignment)
;;(get-model)
(exit)
